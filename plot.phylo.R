...
plot.phylo <-
    function(x, type = "phylogram", use.edge.length = TRUE,
             ...
             y.lim = NULL, direction = "rightwards", 
             lab4ut = ifelse(type=="unrooted","horizontal", "radial"),
...
# added open.angle, Klaus         
        ...
        ## radius:
        X <- 1 - X/Ntip
        Y <- .nodeHeight(Ntip, Nnode, z$edge, Nedge, theta)
...
# added label.offset
            ...
            pin1 <- par("pin")[1] # width of the device in inches            
            strWi <- max(strwidth(x$tip.label, "inches", cex=cex)) 
            xx.range <- diff(range(xx)) #* 1.04
            alp <- try(uniroot(function(a) max(a*xx.range + strWi) - pin1,
                               c(0, 1e6))$root, silent = TRUE)
            ## if the above fails, give 1/3 of the device for the tip labels:
            if (is.character(alp)) offset <- max(xx.range) / 3 else {
                offset <- if (show.tip.label)  strWi/alp  else 0    
            }
            if(lab4ut=="radial") offset <- offset + label.offset
            switch(type, "fan" = {
                if (show.tip.label) x.lim <- c(min(xx) - offset, max(xx) + offset)
                else x.lim <- c(min(xx), max(xx))
        }, "unrooted" = {
            if (show.tip.label) x.lim <- c(0 - offset, max(xx) + offset)
            else x.lim <- c(0, max(xx))
        }, "radial" = {
            if (show.tip.label) x.lim <- c(-1 - offset, 1 + offset)
            else x.lim <- c(-1, 1)
        })
        }
...
# added cex=cex                
            ...
            pin2 <- par("pin")[2]           
            strWi <- max(strwidth(x$tip.label, "inches", cex=cex)) 
            strHi <- max(strheight(x$tip.label, "inches", cex=cex)) 
            yy.range <- diff(range(yy)) #* 1.04
            alp <- try(uniroot(function(a) max(a*yy.range + strWi) - pin2,
                               c(0, 1e6))$root, silent = TRUE)
            if (is.character(alp)) offset <- max(yy.range) / 3 else {
                offset <- if (show.tip.label)  strWi/alp  else 0    
            }
            if(lab4ut=="radial") offset <- offset + label.offset
            else offset <- ifelse(is.character(alp), .03, strHi/alp) 
            
            switch(type, "fan" = {
                if (show.tip.label) y.lim <- c(min(yy) - offset, max(yy) + offset)
                else y.lim <- c(min(yy), max(yy))
        }, "unrooted" = {
            if (show.tip.label) y.lim <- c(0 - offset, max(yy) + offset)
            else y.lim <- c(0, max(yy))
        }, "radial" = {
            if (show.tip.label) y.lim <- c(-1 - offset, 1 + offset)
            else y.lim <- c(-1, 1)
        })
        }
...
#        if (type == "unrooted") {
            ...
            if (type == "unrooted") angle = XY$axe
            else angle <- atan2(yy[1:Ntip], xx[1:Ntip])
...
# avoids which ==> faster for large trees
# e.g. tree with 10000 taxa 5.5 -> 2.5 sec.
NodeInEdge <- function(edge, nTips){
    edge = edge[,1]
    res <- vector("list", max(edge) - nTips)
                           ...
                           edge.color, edge.width, edge.lty)
{
    ...
    NodeInEdge1 <-NodeInEdge(edge, Ntip)
     for(i in 1:length(NodeInEdge1)){
        j <- NodeInEdge1[[i]] 
...
#   changes:  
#   lab4ut now works also for fan and radial
#   open.angle also works also for radial  
#   phylogram.plot is faster for very large trees (10000 taxa)
#
#   tip.labels seem more often inside the margins, 
#       especially when setting label.offset and cex
#   lab4ut="horizontal" is not wasting space below and above 
#   personally I am not a fan of introducing side effects 
#       no.margin = FALSE
# 
# TODO srt likely out of margin
